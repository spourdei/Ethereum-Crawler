...
class Transaction(BaseModel):
    ...
    blockHash: Optional[str] = (
        None  # optional since it can be None for pending transactions
    )
    blockNumber: Optional[str] = None
    from_: str = Field(
        alias="from"
    )  # using alias to handle conflict with "from" for Python
    to: Optional[str] = (
        None  # optional, as it could be None when its a contract creation transaction
    )
    value: str
